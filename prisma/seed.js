...
async function main() {
  ...
  const affirmationsData = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  ...
  const affirmations = affirmationsData.map((aff) => {
    ...
    const isPremium =
      aff.isPremium !== undefined ? aff.isPremium : parseInt(aff.id) % 5 === 0;
